...
最近本机部署见 [1.1.1 分析稳定性修复替换记录](deployments/2026-09-13-analysis-reliability.md)：Release 构建和 402 项测试通过，已覆盖到 `D:\data\avalonia\Controls\NovelGeneratePlugin`，旧插件已备份。历史记录：[1.1.0 整本小说分析](deployments/2026-09-13-novel-analysis.md) · [1.0.1 DeepSeek 默认连接](deployments/2026-09-12-deepseek-defaults.md) · [Controls 初始部署](deployments/2026-09-12-local-controls.md)。
...
<!-- 2、3. src/NovelGeneratePlugin.Plugin/NovelGeneratePlugin.Plugin.csproj：引用并声明由插件携带 -->
<ItemGroup>
...
- 用 `dotnet list src/NovelGeneratePlugin.Plugin/NovelGeneratePlugin.Plugin.csproj package --include-transitive` 查看传递依赖；
...
`Controls/NovelGeneratePlugin/` 内。
...
该目标只重建 `Controls/NovelGeneratePlugin`，不会清理 `Controls` 根目录或其他插件。需要给开发目录增加醒目标记时，
可覆盖目录名：
...
dotnet msbuild src/NovelGeneratePlugin.Plugin/NovelGeneratePlugin.Plugin.csproj `
  ...
  -p:Configuration=Debug `
  -p:ManagedPluginDeployRoot=C:\Path\To\Host\Controls `
  -p:ManagedPluginDirectoryName=NovelGeneratePlugin-Dev
```

`NovelGeneratePlugin-Dev` 只是文件夹名称，插件身份仍是 manifest 中的 `myavalonia.plugin.novel.generate`。
...
dotnet msbuild src/NovelGeneratePlugin.Plugin/NovelGeneratePlugin.Plugin.csproj `
  -t:DeployManagedPlugin `
  -p:Configuration=Debug `
  -p:ManagedPluginDeployRoot=C:\Temp\NovelGeneratePlugin-Deploy\Controls
```

然后把整个 `C:\Temp\NovelGeneratePlugin-Deploy\Controls\NovelGeneratePlugin` 复制到 Host 的 `Controls` 下。目标叶子目录可以
改为 `NovelGeneratePlugin-Dev`，但必须遵守：

- 把插件目录作为一个整体替换，不要把新文件合并覆盖到旧目录，否则删除过的依赖可能残留；
- 同一个 Host 中只保留一份 `myavalonia.plugin.novel.generate`，不能同时留下 `NovelGeneratePlugin` 和 `NovelGeneratePlugin-Dev`；
...
```text
src/NovelGeneratePlugin.Plugin/artifacts/managed-plugin-packages/
├─ NovelGeneratePlugin.Plugin-1.0.0-win-x64.zip
└─ NovelGeneratePlugin.Plugin-1.0.0-win-x64.manifest.json
```

ZIP 内保持 `Controls/NovelGeneratePlugin/` 布局；同名外置 `.manifest.json` 记录 ZIP 和文件摘要。正式交付时让二者
