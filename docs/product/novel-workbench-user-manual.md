...
当前开发版在完整报告页提供“从报告生成模板”。默认提炼世界观、文风、写作方法，也可选入长期规则。填写名称、模型连接和本次预算，可以先点“预览转换计划”，再点“生成新模板草案”；直接生成也会先进行容量检查。
...
重开后可从模板下拉列表选择已保存的草案或版本；初始空白编辑器不表示库中没有模板。打开生成草案成功时，会提示“已打开生成草案，可编辑后保存新版本”。本轮功能尚未部署到安装目录，工程和真实模型验收状态分别见[本次验收记录](../refactoring/G0044/acceptance-matrix.md)。
