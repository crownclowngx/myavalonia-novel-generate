...
适用版本：2026-09-12 首版工程候选。本文按当前可用界面编写，帮助你完成从准备素材到导出正文的操作。软件尚未正式发布；完整桌面体验和作者质量签收仍在验收中。
...
3. 分别填写规划、正文、检查的模型预设。本版实测组合是 `gpt-6-astra`、输出上限 `8192`、推理强度 `low`，对应 CLI 版本为 `0.153.4`。这是本版验证记录，具体可用模型以你账户支持的配置为准。
4. 点击“保存配置”。若执行单句生成检测，会实际消耗套餐额度；先确认检测结果与用量。
...
DeepSeek 使用独立 API 密钥连接，与 Codex 套餐分开。本说明书以已验证的 Codex 路径为例；DeepSeek 远端实测不属于本版已完成的验收内容。
...
| 推理强度 | 传给支持该选项的模型的思考设置 | 在连接的任务预设中设置；本版实测 low。它不是文风旋钮，不能替代明确章纲 |
...
本版提供短材料、近期章纲与可控范围的连续创作；整本参考小说自动提炼、外部小说批量拆章导入和长期完整知识库仍不在当前操作范围内。本说明书不会要求你寻找尚不存在的按钮。
