...
可以直接粘贴文本，也可以填写 TXT / Markdown 文件路径后点“读入短文字文件”。材料最多 20000 字符，单次分析最多处理前 12000 字符，实际范围以面板“全文 / 本次处理 / 未处理”提示为准。这个短材料入口只处理选定片段；整本 TXT 的分析和模板提炼请使用第 9 节的“整本小说分析”。PDF / Word / EPUB 解析仍未接入。
...
已有 TXT、Markdown 或 Word 正文：若目标是分析参考作品并提炼创作规范，可使用第 9 节的整本 TXT 分析；这不会创建正在创作的作品章节。要把旧稿接入创作项目并续写，仍需在原编辑器打开正文，再按下面步骤逐章接入；原稿另行保留。
...
当前开发版提供短材料、近期章纲、可控范围的连续创作、独立 TXT 分析报告和报告转创作模板。外部小说批量转成创作章节、具体人物历史自动建档和长期完整知识库仍属后续范围；安装目录中的实际版本以部署记录为准。
...
3. 在“运行”选择参考小说、已有模型连接。勾选“根据模型上下文自动分批”，点击“预览分析批次”查看首轮请求数，再核对请求和 token 总额，点击“开始新的全文分析”。所有提取、跨章整合、汇总、专题和综合请求都计入同一预算；阶段参数可在“分析专用参数”中调整。
...
在“运行 → 分析专用参数”设置五个阶段的模型、思考强度和单次输出上限。这些参数不修改创作连接；不勾选时使用原检查预设。新建自动分批参数时，DeepSeek 提取默认使用 none、65536，汇总使用 none、16384，整合/专题/综合使用 high、32768。旧运行继续读取原参数；切换分批方式不会自动覆盖已填写的阶段参数。
...
### 9.4 提示某个 Evidence 数量超限

新版会显示具体位置，例如“$.Entities[1].Evidence：条目数量须为 1–8，实际 11”。这表示第二个实体给了 11 条出处，超过每项 8 条的上限，不是累计 token 预算不足。证据应选少量有力出处，不必把人物所有出现位置都列出。

新建提取节点会收到明确的数量、文本长度和段号约束。完整且用量已知的错误最多自动纠正一次；仍失败时保留候选，不会静默删掉多余证据。已有运行采用原提示，要使用新版完整约束，应保留原范围新建修订，成功节点继续复用。

### 9.5 分析保存在哪里，关闭后怎样继续查看

原文快照、分析进度、专题和综合结论会自动保存在本机插件工作区，默认位于当前用户的 LocalAppData 下的 NovelGeneratePlugin 目录，与 Controls 插件安装目录分开。导出 Markdown 是另存一份文件，不是保留分析结果的前提。

重新打开“整本小说分析”，选择已导入小说和已保存运行，点击“读取所选运行的报告”。较早的运行可以通过“加载更早的已保存运行”找回。进度显示已保存节点和真实检查点时间；部分报告会明确标记，尚未返回或落盘的输出不算已保存。

删除最初的外部 TXT 不会删除导入快照；报告阅读无需模型在线。删除整个插件数据目录会丢失其中的资产，不等于删除原 TXT。

### 9.6 将已保存报告自动变成模板

1.2.0 在完整报告页提供“从报告生成模板”。默认提炼世界观、文风、写作方法，也可选入长期规则。填写名称、模型连接和本次预算，可以先点“预览转换计划”，再点“生成新模板草案”；直接生成也会先进行容量检查。

系统使用已保存专题与综合结论，不重新分析整本小说。输入超容量会分组转换并归纳；每个成功步骤自动保存。转换请求和 token 用量与原全文分析分开显示，金额未核定时显示未知。

完成后先查看规范预览及来源依据，再点“打开生成草案”。若正在编辑别的模板，先保存或放弃那份编辑；生成草案已独立保存在库中，不需要重新生成。编辑后点击“保存新版本”，作品的模板选择器会刷新，再按需要预览并采用相应字段。空字段默认不勾选；主动采用空字段会清空本书对应内容，差异预览会提示。

“来源明确支持”“归纳推断”“创作建议或待确认”分别展示。长期规则中的推断或建议只留在预览，不自动写入持续约束；需要采用时可在草案里自行补充。

在“此报告的已保存转换”中可重开历史、查看来源或继续失败任务。继续使用其已保存模型配置，可以提高总预算；更换模型或单次容量请生成新模板。遇到未知费用或失败候选，先阅读诊断和费用再明确继续。AI 已返回而本地保存失败时优先补保存，模板已创建但任务登记失败时核对同一模板，避免重复生成或覆盖后续编辑。

重开后可从模板下拉列表选择已保存的草案或版本；初始空白编辑器不表示库中没有模板。打开生成草案成功时，会提示“已打开生成草案，可编辑后保存新版本”。1.2.0 与配套 Host 退出修复已[部署到本机安装目录](../deployments/2026-09-13-report-template.md)，重新启动宿主后生效。工程和真实模型验收状态分别见[本次验收记录](../refactoring/G0044/acceptance-matrix.md)。

### 9.7 根据模型上下文自动分批

新面板默认启用自动分批；读取旧分析时会恢复旧模式。要让已导入小说使用新方式，在“运行”勾选“根据模型上下文自动分批”，新建分析或修订即可，不必重新导入 TXT。

每批目标输入默认 200000 token，可在 8192–800000 之间调整。已知 DeepSeek Flash/V4 模型按 1000000 token 上下文计算，未知模型需填写真实容量。目标是包含提示和结构定义的保守估算，不是承诺每批都使用这么多 token；输出预留、模型窗口和内容密度可能使批次更小。当前仍使用 UTF-8 字节数加余量估算，实际用量以服务端返回为准。

连续章节可以合为一个请求，长章节可自动拆分。每个批次内的章内片段仍分别返回六维分析和原文证据，不把所有章节压成同一份固定条数的提取结果。手动模式中的 12000 字符限制不限制自动批次。整本来源的 200 万 UTF-16 字符、20 MiB 文件容量仍适用。

“预览分析批次”只做本地规划，不创建运行或发送模型请求。开始分析时按当前参数重新计算；单次请求超时默认 15 分钟，可设为 1–15 分钟。输入预检超限、用量已知的输出截断仍可在拆分深度和总预算内自动二分。用量未知或断流时保留候选并等待复核。

自动分批修订会保留已完成批次，只重新规划未完成的原文范围。成功的批次和费用在暂停、重启后继续保留。全过程 token 上限与单次输入容量分别计算，自动分批不会取消累计预算。

自动分批功能已随 1.3.0 [发布到本机安装目录](../deployments/2026-09-14-automatic-batching.md)，重新启动宿主后生效。
