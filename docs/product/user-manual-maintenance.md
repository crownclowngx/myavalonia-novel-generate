...
- [用户说明书 HTML](novel-workbench-user-manual.html)：普通作者直接打开阅读，五条使用路线、七张实际界面截图、目录跳转、图片放大、打印样式；正文和图片均内嵌，无外部网络依赖。
...
说明书 v1.1 增加 DeepSeek 只填写 Key 的流程、模型与思考下拉选项及第七张实际截图。图片来自本次 Release 原生界面测试 `deepseek-defaults.png`，保持 Key 为空；此前六张截图仍用于对应创作场景。参数依据与边界见 [DeepSeek 默认连接说明](../deepseek-defaults.md)。
