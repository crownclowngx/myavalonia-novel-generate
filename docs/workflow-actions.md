...
当前模板精确引用 Plugin SDK `3.4.0`；其中的 Workflow Action 契约保持兼容。独立的 Workflow SDK `1.0.0`
提供 Schema、引用路径、
...
- 目标 Host 允许同一插件同时登记 Provider 与 Consumer，但禁止自调用，以及 Handler 内嵌套调用 Gateway。
  当前小说插件未登记任何 Workflow 能力；G0023 实施时复核目标 Host 契约并通过真实 ZIP 验证。
