    ...
    public async Task<SaveOutcome> SaveAsync()
    {
        ...
        try
        {
            ...
            catch (Exception exception) when (exception is not OutOfMemoryException)
            {
                ...
                try
                {
                    await Task.Run(() => _recovery.Write(RecoveryPath, new RecoverySnapshot(3, Path, databaseVersion, snapshot, DateTimeOffset.UtcNow, editVersion))).ConfigureAwait(false);
