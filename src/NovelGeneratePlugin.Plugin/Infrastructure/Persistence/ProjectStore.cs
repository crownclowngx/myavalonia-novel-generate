...
public sealed class ProjectStore : IProjectStore
{
    ...
    public StoredProject Read(string path)
    {
        using (var connection = Connect(path, SqliteOpenMode.ReadOnly))
        {
            ...
            if (version != 1) throw new NotSupportedException("项目格式版本不受支持，未进行迁移或写入。");
    ...
    private static void MigrateFromV1(string path)
    {
        using var source = Connect(path);
        if (ReadSchema(source) != 1) throw new InvalidOperationException("迁移前项目版本已变化，请重新打开。");
        var backupPath = path + ".before-v2-" + Guid.NewGuid().ToString("N") + ".noveldb";
        ...
        if (ReadSchema(source, transaction) != 1) throw new InvalidOperationException("事务开始时项目格式已变化，迁移已中止。");
        var original = ReadSnapshot(source, transaction);
        using var command = source.CreateCommand(); command.Transaction = transaction;
        command.CommandText = "UPDATE project SET snapshot=$snapshot, version=version+1 WHERE singleton=1; PRAGMA user_version=2;";
