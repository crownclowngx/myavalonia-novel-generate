...
public sealed class ProjectStore : IProjectStore
{
    ...
    public StoredProject Read(string path)
    {
        using (var connection = Connect(path, SqliteOpenMode.ReadOnly))
        {
            ...
            if (version is not (1 or 2)) throw new NotSupportedException("项目格式版本不受支持，未进行迁移或写入。");
    ...
    private static void MigrateKnownFormat(string path)
    {
        ...
        if (expectedSchema is not (1 or 2)) throw new InvalidOperationException("迁移前项目版本已变化，请重新打开。");
        var backupPath = path + ".before-v3-" + Guid.NewGuid().ToString("N") + ".noveldb";
        ...
        command.CommandText = "UPDATE project SET snapshot=$snapshot, version=version+1 WHERE singleton=1; PRAGMA user_version=3;";
