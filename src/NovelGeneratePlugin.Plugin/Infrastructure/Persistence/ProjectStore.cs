...
public sealed class ProjectStore : IProjectStore
{
    ...
    public StoredProject Read(string path)
    {
        using (var connection = Connect(path, SqliteOpenMode.ReadOnly))
        {
            ...
            if (version is not (1 or 2 or 3 or 4 or 5 or 6 or 7 or 8)) throw new NotSupportedException("项目格式版本不受支持，未进行迁移或写入。");
    ...
    private static void MigrateKnownFormat(string path)
    {
        ...
        if (expectedSchema is not (1 or 2 or 3 or 4 or 5 or 6 or 7 or 8)) throw new InvalidOperationException("迁移前项目版本已变化，请重新打开。");
        var backupPath = path + ".before-v" + SchemaVersion + "-" + Guid.NewGuid().ToString("N") + ".noveldb";
        ...
        command.CommandText = $"UPDATE project SET snapshot=$snapshot, version=version+1 WHERE singleton=1; PRAGMA user_version={SchemaVersion};";
    ...
    public long Save(string path, BookProject project, long expectedVersion)
    {
        ...
        project.Planning.EnsureAppendOnlyFrom(previous.Project.Planning);
        using var command = connection.CreateCommand(); command.Transaction = transaction;
