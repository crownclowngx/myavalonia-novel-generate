...
public sealed class ProjectStore : IProjectStore
{
    ...
    public StoredProject Read(string path)
    {
        ...
        using var command = connection.CreateCommand(); command.CommandText = "SELECT id, version, snapshot FROM project WHERE singleton=1";
    ...
    private static void ValidateSchema(SqliteConnection connection)
    {
        ...
        if (Convert.ToInt32(command.ExecuteScalar()) != SchemaVersion) throw new NotSupportedException("项目格式版本不受支持，未进行迁移或写入。");
    }
