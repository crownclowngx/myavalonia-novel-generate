...
for name, renderer in [('novel-generation-product-requirements', 'render-product-html.py'),
                       ('novel-workbench-user-manual', 'render-user-manual.py')]:
    source = root / f'docs/product/{name}.md'
    ...
    if f'name="source-sha256" content="{expected}"' not in rendered:
        errors.append(f'{name} HTML 未与 Markdown 同步，请运行 python docs/product/{renderer}')
if errors:
    raise SystemExit('\n'.join(errors))
print(f'Markdown local links passed: {count}')
print('Product and user manual Markdown/HTML source hashes passed')
